/**
 * PrepaidService — the prepaid inbound-purchase path, as an indivisible brick alongside JitService.
 *
 * A client pays a CKB fee up front and the LSP then opens the channel:
 ...
 *
 * This path is *trusted by construction* — nothing atomically binds the fee to the open (see
 * docs/ARCHITECTURE.md). JIT exists precisely to remove that trust; the two are separate bricks because they
 * are separate lifecycles, and the API composes whichever the deployment mounts.
 *
 * Every side effect goes through FiberChannelRpcClient, which can be backed by a replay transport for offline
 * tests — the same real code path runs in the demo against a live FNN node.
 */
import {
  type AssetOffering,
...
export interface PrepaidServiceConfig {
  rpc: FiberChannelRpcClient;
  /** Named on `from_capacity` payments so the client knows which node to fund. */
  lspPubkey: string;
  ...
  /**
   * Confirm the fee invoice actually settled before opening the channel. Defaults to trusting the explicit
   * settleFee() call (fine for the demo). A production LSP injects a check that queries the node's invoice
   * status so a client can't get a channel without paying — see `makeInvoiceFeeVerifier`.
   */
...
export class PrepaidService {
  private readonly cfg: Required<
    Pick<
      PrepaidServiceConfig,
      "orderExpirySeconds" | "readyPollAttempts" | "readyPollIntervalMs" | "now" | "idgen" | "sleep" | "store"
    >
  > &
    PrepaidServiceConfig;

  constructor(config: PrepaidServiceConfig) {
    this.cfg = {
