...
## Reference run (real output, CKB testnet)

```
$ node scripts/demo/00-setup.mjs
=== STEP 0 · Top up the customer with 5 RUSD of outbound ===
   customer RUSD spendable before: 0.996
   customer RUSD spendable after:  5.996  ✓

$ node scripts/demo/01-discover-lsp.mjs
=== STEP 1 · Discover an LSP selling RUSD inbound ===
   [gossip graph]  50 node(s) advertise RUSD auto-accept ≤ 10 RUSD — the decentralized capability landscape:
      • 029c394864e383990dc3…   floor 10 RUSD
      … and 45 more
   [registry]      resolving an orderable endpoint + price…
   ✅ orderable LSP http://127.0.0.1:8080  (lsp 023dda5d534934…)
      quote: fee 1000000000 shannons CKB for 10 RUSD inbound

$ node scripts/demo/02-buy-inbound.mjs
=== STEP 2 · Take the offer → open a public RUSD channel for 10 RUSD inbound (merchant funds 0) ===
   [fee] pay 1000000000 shannons CKB out-of-band  (invoice fibt10000000001pasf7…)
   order 86b84139-… → channel_active
   ✅ channel_active — outpoint 0xcb993a18ee90c8acf92d9dfed0f6c6d74e2abf7197f410008ec7aa730b1aa1f4…
   merchant now holds 10 RUSD of inbound it can receive over (funded 0)

$ node scripts/demo/03-invoice.mjs
=== STEP 3 · Merchant issues an invoice for 3 RUSD ===
   invoice:      fibt3000000001pp…
   payment_hash: 0xace811cacc567fa4…

$ node scripts/demo/04-pay.mjs
=== STEP 4 · Customer pays 3 RUSD — routed customer → LSP → merchant ===
   payment: Success  (fee 0x493e0)
   customer RUSD spendable: 5.996 → 2.993   (paid 3.003)
   merchant RUSD spendable: 4 → 7   (received 3)
   → the gap (0.003 RUSD) is the LSP's forwarding fee: proof it routed through the hub
   [webhook] invoice.paid → receipt 60d08df7-… · paid=true
   [ledger] accounting CSV:
     receipt_id,status,paid,asset,amount,fee_paid,issued_at,settled_at,payment_hash,description
     60d08df7-…,Paid,true,RUSD,300000000,,2026-07-04T…,2026-07-04T…,0xace811ca…,demo order
